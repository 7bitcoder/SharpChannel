...
namespace cm
{
    ...
    {
    public:
        std::unique_ptr<IUnsubscribable> subscribe(const OnCompleted &onCompleted, const OnConnected &onConnected = OnConnected(), const OnError &onError = OnError()) final;

        std::unique_ptr<IUnsubscribable> subscribe(const OnMessageReceived &onMessageReceived, const OnConnected &onConnected = OnConnected(), const OnCompleted &onCompleted = OnCompleted(), const OnError &onError = OnError()) final;
        std::unique_ptr<IUnsubscribable> subscribe(const OnDataReceived &onDataReceived, const OnConnected &onConnected = OnConnected(), const OnCompleted &onCompleted = OnCompleted(), const OnError &onError = OnError()) final;
