...
namespace cm
{
    ...
    struct IReadOnlyChannel : public virtual IRunnable
    {
        ...
        virtual std::unique_ptr<IUnsubscribable> subscribe(const OnCompleted &onCompleted, const OnConnected &onConnected = OnConnected(), const OnError &onError = OnError()) = 0;

        virtual std::unique_ptr<IUnsubscribable> subscribe(const OnMessageReceived &onMessageReceived, const OnConnected &onConnected = OnConnected(), const OnCompleted &onCompleted = OnCompleted(), const OnError &onError = OnError()) = 0;
        virtual std::unique_ptr<IUnsubscribable> subscribe(const OnDataReceived &onDataReceived, const OnConnected &onConnected = OnConnected(), const OnCompleted &onCompleted = OnCompleted(), const OnError &onError = OnError()) = 0;
